...
/**
 * Выбор значений одного измерения критериев внутри конкретной матрицы.
 * mode 'all' — измерение не ограничивает применение матрицы (применяется
 * ко всем значениям списка); 'specific' — матрица применяется только
 * к перечисленным selectedItemIds (id элементов из Reference Data Engine).
 */
export interface MatrixCriteriaSelection {
  dimensionId: string;
  mode: 'all' | 'specific';
  selectedItemIds: string[];
  /** Является ли соответствие этому измерению обязательным условием применения матрицы. */
  mandatory: boolean;
}
