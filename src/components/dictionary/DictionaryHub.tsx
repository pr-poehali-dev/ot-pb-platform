    ...
    <div>
      ...
      <header className="mb-8">
        ...
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          Центральный модуль платформы Noventra. Единый источник данных и единый CRUD-интерфейс для всех бизнес-модулей.
        </p>
      </header>

      {/* Stats strip */}
      <div className="mb-10 grid grid-cols-2 gap-4 sm:grid-cols-4">
        <StatCard icon="Library" label="Справочников" value={String(DICTIONARIES.length)} />
        <StatCard icon="Database" label="Всего записей" value={String(totalItems)} />
        <StatCard icon="Star" label="В избранном" value={String(favorites.length)} />
        <StatCard icon="History" label="Недавние" value={String(recent.length)} />
      </div>
      ...
      <section className="mb-10">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h2 className="font-display text-lg font-semibold">Часто используемые</h2>
        </div>
      ...
      {favoriteConfigs.length > 0 && (
        <section className="mb-10">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="font-display text-lg font-semibold">Избранные справочники</h2>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
      ...
      {recentConfigs.length > 0 && (
        <section>
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="font-display text-lg font-semibold">Последние открытые</h2>
          </div>
